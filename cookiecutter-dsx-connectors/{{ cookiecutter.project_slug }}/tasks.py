...
name = '{{ cookiecutter.__release_name }}'
...
def prepare(c):
    ...
    c.run(f"mkdir -p {export_folder}/connectors/{{ cookiecutter.project_slug }}")
    c.run(f"mkdir -p {export_folder}/dsx_connect/models")
    c.run(f"mkdir -p {export_folder}/dsx_connect/utils")

    # Copy connectors
    c.run(f"rsync -av --exclude '__pycache__' {project_root_dir}/connectors/{{ cookiecutter.project_slug }}/ {export_folder}/connectors/{{ cookiecutter.project_slug }}/ --exclude 'deploy' --exclude 'dist' --exclude 'tasks.py'")
